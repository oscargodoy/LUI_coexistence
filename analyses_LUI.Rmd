...
#number of species per plot
rich_50spp <- apply(plants, 1, sum, na.rm = TRUE)

#merge lui and richness
richness_50spp_comm <- cbind(lui_total, rich_50spp)

#year as factor for plotting
richness_50spp_comm$Year <- as.factor(richness_50spp_comm$Year)

#save full community presence data
write.table(richness_50spp_comm, "data/richness_50spp_comm.txt",
            row.names = FALSE, sep = "\t")

#clean
rm(lui_total, plants, rich_50spp)
```
...
lui_rich_50spp <- ggplot(data = richness_50spp_comm,
                         aes(x = LUI, y = rich_50spp, colour = Year)) +
  geom_point(alpha = 0.15) + geom_smooth(alpha = 0, size = 1.1) +
  scale_x_continuous(limits = c(0.5, 3), breaks = seq(0.5, 3, by = 0.5)) +
  scale_y_continuous(position = "left") +
  ylab("Richness (50-species community)") +
  theme(legend.position = "none") +
...
both_lui_rich <- ggarrange(lui_rich_full, lui_rich_50spp,
                           nrow = 2, align = "hv",
                           labels = c("B", "C"),
...
(ratio_combos <- ggplot(data = saturation, aes(x = richness, y = ratio)) +
    scale_x_continuous(breaks = c(2, 3, 5, 7, 11, 17, 25, 37,42)) +
    ggtitle("Community of 50 species") +
...
#load coefficients
alpha <- as.matrix(read.table("results/interaction_matrix_lme_average_51.txt", header = TRUE, sep = "\t"))
lui_modify_alpha <- as.matrix(read.table("results/lui_matrix_lme_average_51.txt", header = TRUE, sep = "\t"))
intrinsic <- as.matrix(read.table("results/intrinsic_site_lui_average_lme_51.txt", header = TRUE, sep = "\t"))
...
combos_lui5 <- readRDS("results/selected_combos/combos5.rds")
combos_lui7 <- readRDS("results/selected_combos/combos7.rds")
...
#compute combinations of 5 species
#results_coex <- list()
#results_coex <- foreach (i = 1:length(combos_lui5), .packages = "mvtnorm") %dopar% {
#  str_coex_combos(alpha = lui_alpha, intrinsic = lui_intrinsic_positive, combos = combos_lui5)
#}
...
#write.csv(coex5, file = "results/results-LUI_coexistence-5spp.csv", row.names = FALSE)

#compute combinations of 5 species
#results_coex <- list()
#results_coex <- foreach (i = 1:length(combos_lui7), .packages = "mvtnorm") %dopar% {
#  str_coex_combos(alpha = lui_alpha, intrinsic = lui_intrinsic_positive, combos = combos_lui7)
#}
...
#write.csv(coex7, file = "results/results-LUI_coexistence-7spp.csv", row.names = FALSE)
...
for (i in 1:length(lui_alpha)){
  ...
  for (j in 1:n){
    ...
    rintrinsic[[j]] <- as.matrix(data.frame(
      ...
      "lui" = lui))
    rownames(rintrinsic[[j]]) <- species_names
