...
At this point, we need to estimate the interaction coefficients and LUI effects. We want to obtain an interaction matrix between all 26 species without the effects of LUI and the effects of LUI, plus the intrinsic growth rate for all the species. **NOTE FOR MYSELF: I NEED TO ADD COMMENTARIES TO EXPLAIN THE STEPS IN THIS PIECE OF CODE!** 
...
Now, we obtain the errors from the linear model. **NOTE FOR MYSELF: I NEED TO ADD COMMENTARIES TO EXPLAIN THE STEPS IN THIS PIECE OF CODE!**
...
```{r prepare lui}
#load coefficients
alpha <- read.table("results/interaction_matrix_lme_average_26.csv", header = TRUE, sep = ",", row.names = 1)
lui_modify_alpha <- read.table("results/lui_matrix_lme_average_26.csv", header = TRUE, sep = ",", row.names = 1)
intrinsic <- as.matrix(read.table("results/intrinsic_site_lui_average_lme_26.csv", header = TRUE, sep = ",", row.names = 1))
...
structural_coex <- function(alpha, intrinsic, n, combos, treatment){
  ...
  results_combos <- matrix(nrow = dim(combos)[1], ncol = (5 + ncol(combn(n, 2)) + 2))
...
combos <- lui_combos2
richness <- 2
path <- "results/coex_LUI_2spp.txt"
...
#save, just in case
results <- results[, c(1, 2, 5, 8, 9)]; rownames(results) <- NULL
...
colnames(coex2) <- c("SND", "SFD", "feasibility", "LUI", "combos")
coex2$feasibility <- as.factor(coex2$feasibility)
coex2$LUI <- as.factor(coex2$LUI)

#load coexistence 2 species results
coex3 <- read.table(file = "results/coex_LUI_3spp.txt", header = TRUE, sep = "\t")
colnames(coex3) <- c("SND", "SFD", "differential", "overlap", "feasibility",
                     "c1_2", "c1_3", "c2_3", "coex_rate", "LUI", "combos")
coex3$feasibility <- as.factor(coex3$feasibility)
coex3$LUI <- as.factor(coex3$LUI)
...
#x = SND, y = SFD, color = LUI
ggplot(data = coex2, aes(x = SND, y = SFD, color = LUI)) +
  geom_point(alpha = 0.4, size = 1.5) +
  ggtitle("Combinations of 2 species")

ggplot(data = coex3, aes(x = SND, y = SFD, color = LUI)) +
  geom_point(alpha = 0.4, size = 1.5) +
  ggtitle("Combinations of 3 species")

#x = LUI, y = SFD
ggplot(data = coex2, aes(x = LUI, y = SFD)) + geom_point() +
  ggtitle("Combinations of 2 species")

ggplot(data = coex3, aes(x = LUI, y = SFD)) + geom_point() +
  ggtitle("Combinations of 3 species")

#x = LUI, y = SND
ggplot(data = coex2, aes(x = LUI, y = SND)) + geom_point() +
  ggtitle("Combinations of 2 species")

ggplot(data = coex3, aes(x = LUI, y = SND)) + geom_point() +
  ggtitle("Combinations of 3 species")

#x = LUI, y = feasibility
ggplot(data = coex2, aes(x = LUI, y = feasibility)) + geom_point() +
  ggtitle("Combinations of 2 species")

ggplot(data = coex3, aes(x = LUI, y = feasibility)) + geom_point() +
  ggtitle("Combinations of 3 species")
