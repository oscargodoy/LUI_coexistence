...
lui.mean <- read.table("data/LUI.08.15.mean.txt", header = TRUE)

library(SDMTools)
plants4 <- aggregate(plants2, list(plants$Useful_EP_PlotID), mean, na.rm = TRUE)
names(plants4)[1] <- "Plot"
plants5 <- melt(plants4)
plants6 <- merge(plants5, lui.mean[, c(1, 5)], by = "Plot", all.x = TRUE)
